...
export const EntryDetailPanel = ({ entry, renderedBodyHtml, entryTags, editHref, deleteHref }: EntryDetailPanelProps) => {
  ...
  if (!entry) {
    ...
    <section class="rounded-2xl border border-dashed border-slate-700 bg-slate-950/90 p-4 text-slate-300 lg:min-h-[calc(100vh-112px)]">
        ...
        <h2 class="mt-1 text-lg font-semibold text-slate-50">{text.detail.noEntryTitle}</h2>
    ...
    <section class="flex min-h-[calc(100vh-112px)] flex-col rounded-2xl border border-slate-700/80 bg-slate-950/90 p-4 shadow-[0_18px_54px_-36px_rgba(2,6,23,0.95)]">
      <div class="flex flex-wrap items-start justify-between gap-3 border-b border-slate-800 pb-3">
        ...
        <div class="text-right">
          <p class="mb-1.5 text-[10px] font-medium tracking-wide text-slate-400">
          ...
          <div class="flex flex-wrap items-center justify-end gap-1.5">
            ...
            {entryTags.map((tagName) => (
              <a
                ...
                class="badge badge-outline border-cyan-300/40 bg-cyan-300/15 transition hover:border-cyan-200/70 hover:bg-cyan-300/25 hover:text-white"
              >
                {tagName}
              </a>
            ))}          </div>
